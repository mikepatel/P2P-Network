...
"""
- A does not begin with any RFCs
- register with RS
- query RS for active peer list
- connect to B and download 1 RFC
...
#
peerNumber = 10 # ONLY MAJOR DIFFERNCE ACROSS DIFFERENT PEER FILES
hostName = 'localhost'
        ...
        try:			
            ...
            for entry in RFC_index_neighbor: # would only want to download RFCs from neighbors
                if entry[0] not in RFC_num_list:
                    ...
                    sleep(10)
                    activePeerList = client.pquery()
                    print('\n', activePeerList)
                    #activePeerList = activePeerList = client.pquery() # peer receives list of active peers from RS
                    #print('\n', activePeerList)
                    #break # after 1 RFC download
